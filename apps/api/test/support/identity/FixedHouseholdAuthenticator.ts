...
export class 記憶上の世帯認証器 implements HouseholdAuthenticator {
  ...
  get 呼ばれた回数(): number {
    return this.#受け取った資格たち.length;
  }

  /** 最後に委ねられた資格。まだ一度も呼ばれていなければ `null`。 */
  get 受け取った資格(): string | null {
    return this.#受け取った資格たち.at(-1) ?? null;
  }

  async authenticate(accessToken: string): Promise<HouseholdId> {
    const 応答 = this.#応答たち[this.#受け取った資格たち.length];
    this.#受け取った資格たち.push(accessToken);
