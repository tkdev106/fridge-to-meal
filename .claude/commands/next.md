...
5. **テストを先に書く。** 設計書の「テスト観点」を上から埋める。ドメイン層・ユースケース層は
   実 DB も実 API も使わない（ADR-002）。落ちることを確認してから実装に移る。
6. **実装する。** 設計書のファイル配置と型に従い、`CLAUDE.md` の4つの約束
   （用語表 / 依存の向き / C-1〜C-16 / 未決事項）を守る。**設計書と違うことをしたくなったら、
