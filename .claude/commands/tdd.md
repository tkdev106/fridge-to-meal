...
1. **対象を確かめる。** 引数が設計書のパスなら、まずそれを読む。`docs/backlog.md` に
   無いものは着手しない（`CLAUDE.md`）。1周で2件やらない。**1周 = 1つの振る舞いのかたまり**
...
4. **一覧を検分する。ここが人の代わりに判断する唯一の場所。**
   ...
   - **設計書があるなら、その6章の規則と7章のエラーが、それぞれ1件以上のケースになっているか**
...
10. **コミットする。** テストと実装を**同じコミットに入れる** — 片方だけの状態は `main` で緑にならない。
    ...
    **`/next` から呼ばれたときは、コミットは `/next` 手順9 に任せてここでは作らない。**
    単独で呼ばれたときは、**`git branch --show-current` が `main` ならコミットせずに止まる** —
    枝を切るのは呼び出し側の仕事である（`docs/workflow.md` 1章）
