...
| **C-4** | **重複は生成時にプロンプトで避け、名称の完全一致のみ既存を参照する。** 既存の献立名をプロンプトに渡して重複を避けさせる（ADR-021）。それでも名称が完全一致した生成結果は、新規保存せず既存の献立を参照し、その要素の `origin` は `'reused'` とする。**意味的な重複（表記の違う同じ料理）は引き続き検出しない**。C-15 により生成と再利用は同居しないため、この参照が同じ提案の中で `mealId` を重複させることはない |
