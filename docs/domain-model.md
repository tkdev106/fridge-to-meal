...
> v0.4: C-2 の「生成」の範囲を明確化。生成が3件に満たない場合の扱いを改めた。
> v0.3: 再利用と生成を混ぜないことに決定（ADR-022）。`Suggestion` の件数の不変条件と第6章を改めた。
...
| **アクセストークン** | `accessToken` | 利用者が認証済みであることを示す文字列。**検証を通ってはじめて世帯を定める**（ADR-031）。発行するのは認証基盤であり、このアプリは検証だけを行う | アカウント |
...
| 種別 | 名前 | 責務 |
| --- | --- | --- |
| ドメインサービス | `MealCoverageService` | 献立の**材料の列**と、現在の**在庫品の名称の列**を受け取り、賄える材料と不足する材料を返す。**判定の対象は主材料だけで、調味料は含めない**（C-16）。純粋関数であり、副作用も永続化も持たない。**在庫品の型そのものは受け取らない** — コンテキストをまたぐ import が禁じられているため、名称に落として渡す（ADR-033） |
...
「見た」は生成と同時に成立する — `Meal` になった献立は必ずすべて提示するため、提示済みフラグは不要（確定事項 C-2）。したがって永続化される事実は調理記録だけであり、2つの一覧はどちらもその有無による絞り込みで実装できる。

## 6. 提案の組み立て — LLM を呼ばずに済ませる

提案は必ずしも生成ではない。**手持ちの献立で今日作れるものが1件でもあれば、それを提示して LLM を呼ばない。** これは費用の話であると同時に、応答速度の話でもある — 再利用だけで組めた提案は待ち時間なしで表示できる。

**再利用と生成を混ぜない**（C-15 / ADR-022）。1つの提案は「作れる既存の献立を並べたもの」か「新しく生成した3件」のどちらかであり、両者が同居しない。作れる既存の献立が1〜2件しかなくても、足りない分を生成で埋めることはしない。**足りないと感じたときに新しい献立を求めるかどうかは、利用者が決める。**
...
flowchart TD
  ...
  D --> E{不足0件のものが何件あるか}
  E -->|1件以上| F["上位3件までを再利用する<br/>提案は1〜3件<br/>LLM 0回"]
  E -->|0件| H["3件すべて生成する<br/>LLM 1回"]
  I["新しい献立を求める明示操作"] --> J["必ず3件を新規生成する<br/>再利用しない<br/>LLM 1回"]
```

作れる既存の献立が4件以上ある場合は、C-12 の順序で並べた上位3件を採る。

手持ちの献立は使うほど増えるので、**使い続けるほど LLM を呼ばない経路に入る確率が上がり、1回あたりの費用と待ち時間が下がっていく**。生成が起きるのは「作れる既存の献立が1件もないとき」と「利用者が新しい献立を明示的に求めたとき」の2つだけである。

### 再利用は隠さない

再利用した献立は `SuggestionEntry.origin` が `'reused'` となり、UI 上も新規生成と区別して示す。印を付けずに出すと「なぜ同じ献立がまた出るのか」としか映らない。**「前に見たこれ、今日の在庫で作れます」という提示にして初めて価値になる。**

### 生成するときは既存の献立名を渡す

LLM に生成させるとき、**すでに手元にある献立の名前を渡して「これらとは違うものを」と指示する**（ADR-021）。とくに、新しい献立を求める明示操作では**直前に提示した献立が必ず避ける対象になる**。そこで同じものが出ては、利用者の要求に応えていない。渡す件数には上限を設ける — 蓄積に比例して入力トークンが増え、せっかくの削減効果を打ち消すため。
...
| **C-2** | **生成した献立はすべて提示する。** 生成件数と提示件数は常に等しい。「生成したが見せていない献立」は存在しない。これにより提示済みフラグが不要になる。**ここでいう「生成した献立」は、腐敗防止層の検証を通って `Meal` になったものを指す。** 検証を通らず破棄された応答や、指示より多く返ったために採用しなかった応答は、献立になっていないため対象外である |
| **C-3** | **献立は生成後に編集できない。** 名称・材料・手順は不変。ユーザーが手を加えたい場合の要件は MVP に存在しない。追加されるのは調理記録のみ |
| **C-4** | **重複は生成時にプロンプトで避け、名称の完全一致のみ既存を参照する。** 既存の献立名をプロンプトに渡して重複を避けさせる（ADR-021）。それでも名称が完全一致した生成結果は、**新規保存せず既存の献立を参照する**。照合と参照はユースケース層が行う（腐敗防止層は既存の献立を知らない）。**意味的な重複（表記の違う同じ料理）は引き続き検出しない** |
| **C-4c** | **C-4 で既存を参照しても、`origin` は `'generated'` のままとする。** `origin` は「この提案でどの経路から来たか」を表す値であり、「新規保存したか」を表す値ではない。生成の経路で得たものは、参照先が既存の献立であっても `'generated'` である。**これにより C-15 の「再利用だけか生成だけか」が保たれ、画面の状態も2つに保たれる**（`screen-design.md` 3.1）。失うのは「以前も出た献立だと利用者に伝える機会」だが、**LLM が避ける指示を破り、かつ名称が完全に一致したときにしか起きず、稀である** |
| **C-4b** | ⚠ **C-11 の除外を、生成結果の一致がすり抜けうる。** C-11 で再利用対象から外した献立と同じ名称を LLM が生成した場合、C-4 により既存が参照され、除外したはずのものが提案に戻る（C-4c により印は付かないので、利用者には新しい献立として映る）。**これは既知の穴であり、稀なので許容する。** 塞ぐには生成結果を破棄して再試行することになるが、費用と待ち時間に見合わない |
| **C-5** | **材料は在庫品を参照せず、文字列として複製する。** `MealIngredient` は `StockItemId` を持たない。在庫品が削除されても献立は壊れない。充足判定は名称の突き合わせで都度行う |
| **C-6** | ⚠ **充足判定は名称の完全一致で行う（MVP）。対象は主材料だけである（C-16）。** 「豚こま肉」と「豚こま」は一致しないものとして扱う。表記ゆれの吸収は食材カタログの別名機能に委ね、それでも漏れるものは不足材料として表示する。**これは既知の割り切りであり、精度の問題が出たときに見直す** |
...
| **C-15** | **1つの提案の中で再利用と生成を混ぜない**（ADR-022）。作れる既存の献立が1件でもあれば、その1〜3件だけを提案として返し、LLM を呼ばない。足りない分を生成で埋めない。生成が起きるのは、作れる既存の献立が0件のときと、利用者が新しい献立を明示的に求めたときだけであり、そのときは**3件の生成を求め、再利用を混ぜない**。生成結果が3件に満たなくても、通った件数でそのまま提案を組む。これに伴い `Suggestion.entries` の件数は「ちょうど3件」ではなく「1件以上3件以下」とする |
| **C-16** | **材料は種別を持ち、充足判定の対象は主材料だけとする**（ADR-023）。`MealIngredient` は `kind: 'main' \| 'seasoning'` を持つ。`MealCoverageService` と `CookableMealFinder` は**主材料だけを在庫と突き合わせ、調味料は賄えるものとしても不足するものとしても数えない**。種別は生成時に決まり、以後変わらない（C-3）。**種別が判別できない材料は主材料として扱う** — 不足材料が1件増えるだけで済み、逆に倒すと在庫にない食材が黙って消える |
...
| Q-3 | ~~提案が3件固定でよいか~~ | **決定済み。** 再利用による提案は1〜3件、生成による提案は常に3件（C-15 / ADR-022）。生成結果が3件に満たない場合も、通った件数で提案を組む。0件のときだけ失敗として扱う（`prompt-design.md` 第12章 論点1） |
