...
> ドラフト v0.3 / 2026-09-07 / 要件定義書 v0.6 に対応
...
| **C-15** | **1つの提案の中で再利用と生成を混ぜない**（ADR-022）。作れる既存の献立が1件でもあれば、その1〜3件だけを提案として返し、LLM を呼ばない。足りない分を生成で埋めない。生成が起きるのは、作れる既存の献立が0件のときと、利用者が新しい献立を明示的に求めたときだけであり、そのときは**必ず3件を生成し、再利用を混ぜない**。これに伴い `Suggestion.entries` の件数は「ちょうど3件」ではなく「1件以上3件以下」とする |
...
| Q-3 | ~~提案が3件固定でよいか~~ | **決定済み。** 再利用による提案は1〜3件、生成による提案は常に3件（C-15 / ADR-022）。生成が3件に満たなかった場合は提案を組まず、再試行を促す（`prompt-design.md` 第12章 論点1） |
