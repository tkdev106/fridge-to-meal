...
> ドラフト v0.5 / 2026-09-07 / 要件定義書 v0.6 に対応
...
| **アクセストークン** | `accessToken` | 利用者が認証済みであることを示す資格。**検証を通ってはじめて世帯を定める**（ADR-031）。発行するのは認証基盤であり、このアプリは検証だけを行う | アカウント |
...
| 種別 | 名前 | 責務 |
| --- | --- | --- |
| ドメインサービス | `MealCoverageService` | 献立と現在の在庫品リストを受け取り、賄える材料と不足する材料を返す。**判定の対象は主材料だけで、調味料は含めない**（C-16）。純粋関数であり、副作用も永続化も持たない |
