...
> ドラフト v0.5 / 2026-09-08 / ADR-001 〜 ADR-026
...
| ADR-026 | 在庫品の識別子をドメイン層のポートで発行する | **提案** |
...
- **結果** — **`StockItemId.ts` の doc の記述（発行はインフラ層に任せる）と食い違う。** この ADR が `承認` になった時点で、その doc を直す。**DB の既定値で採番する形に倒すなら、`save` が採番結果を返す必要があり、`StockItemRepository`（B-03）の interface に波及する** — 承認の判断はそこを含む。集約が増えるたびに同じ形のポートが1つずつ増えるが、**`shared/domain` に1つ置いて共有すると、どの集約の識別子かを型で区別できなくなる**ため、重複のほうを採る。
