...
**`main` がトランクであり、常にリリース可能な唯一の幹。** 長命ブランチを作らない。

| 決めごと | 内容 |
| --- | --- |
| 幹 | `main` の1本のみ。`develop` / `release/*` は作らない |
| 枝 | 短命ブランチ1本 = 1 PR = 1タスク。**寿命は原則1日以内** |
...
| 同期 | 枝が1日を超えそうなら、まず分割する。分割できないなら `main` を取り込んで進める |
| 未完成の機能 | 枝を伸ばさず、**未完成のまま `main` に入れて画面から導線を出さない**。枝の寿命 > 機能の完成度 |

**なぜ枝を短くするか:** ループは1周ごとに `main` から出発する。枝が長生きすると、次の周の
出発点が古くなり、コンフリクトの解消がループの主な仕事になってしまう。1周1 PR は
「AI の作業単位」ではなく「トランクを壊さないための上限」である。
