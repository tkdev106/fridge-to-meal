...
画面に出せる状態でなくてもマージする。**隠す仕組み（feature flag）は置かない。**

**本番の配信先がまだ無いため、未完成を見せない相手が居ない。** 隠す仕組みを置くと、
フラグの消し忘れ・ビルドでの畳み込み・読み出しを1か所に縛る lint といった、
**仕組み自体の保守が仕事になる**。守るものが無いうちは、その費用だけが残る。

| 決めごと | 内容 |
| --- | --- |
| 未完成の画面 | **そのまま出す。** `pnpm dev` で見えることは確かめる助けになる |
| サーバ側の未完成 | 隠さない。到達させたくなければ `main.ts` で結線しない |
| 環境変数による分岐 | 置かない。`apps/web` で `import.meta.env` を読む理由がいまは無い |
| 見直す時期 | **本番へ配信する日。** そのとき改めて ADR を起こして決める |

**マージの条件は「完成したか」ではなく「壊していないか」**（上の表）。これはフラグの有無に
関わらず成り立つ — 誰も見ていない段階では、未完成が `main` に載っていて困る相手が居ない。
