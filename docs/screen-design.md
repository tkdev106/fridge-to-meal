...
> ドラフト v0.1 / 2026-09-07 / 要件定義書 v0.6・ドメインモデル v0.4・ADR-001〜022 に対応
> 閲覧用 HTML は未作成。**この Markdown が正である。**

要件定義 第7章の画面一覧を、実装できる粒度まで落とす。**ADR-022 が前提条件として要求した「新しい献立を求める導線」を、ここで具体化する。**

ワイヤーは枠線で示す。**寸法・色・書体は決めない** — 決めるのは何がどこにあるか、どの状態を持つか、どの要件に対応するかである。
...
| | 印 |
| --- | --- |
| 再利用（`origin: 'reused'`） | 🔖「前に見た献立」 |
| 生成（`origin: 'generated'`） | **なし** |

両方に印を付けると、印が背景になって消える。**片方だけに付けるから目に入る。**

印の文言は「前に見た献立」とする。要件 FR-35 の意図は「**前に見たこれ、今日の在庫で作れます**」を伝えることなので、印と「不足なし」の並びでその意味を作る。

> ⚠ **C-4 により、生成した献立が既存と同名だった場合も `origin` は `'reused'` になる。** 利用者から見れば「前に見た献立」であり、表示としては正しい。**実装者が混乱しないよう、ここに書いておく。**
...
- 献立の名称
- 「材料n件 · 不足なし」または「材料n件 · **不足2件**」
- 「使う:」に続けて、**期限が近い在庫を優先して2〜3件**（FR-18 の結果を見せる）
...
- 常備調味料は材料に出ない（prompt-design D-1）。**手順の中に分量とともに書かれている** — この設計が UI に現れるのはここである
- 「これを作った」は**取り消せない**（C-3 / 調理記録は追加のみ）。ただし**確認ダイアログは出さない** — 押し間違えても実害がなく、1タップの重さを増やすほうが害が大きい
- 記録済みの献立を再度開いた場合、ボタンは「もう一度作った」になる（FR-31）
...
### 論点3　「新しい献立を見る」の文言　`未確定`

本書では仮に「新しい献立を見る」としている。禁止語（「候補」「レシピ」「メニュー」）を避けつつ、**押すと数十秒待つことが伝わる**必要がある。実装時に詰める。
